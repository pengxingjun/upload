package com.elfinder.controller;
...
import com.elfinder.controller.executor.CommandExecutionContext;
    ...
    private HttpServletRequest parseMultipartContent(final HttpServletRequest request) throws Exception {
        ...
        while (iter.hasNext()) {
            ...
            if (item.isFormField()) {
                ...
                requestParams.put(item.getFieldName(), Streams.asString(stream, characterEncoding));
                stream.close();
            } else {
                // it is a file!
