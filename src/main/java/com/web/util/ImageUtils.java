...
public class ImageUtils {
     ...
     * @param bb     比例不对时是否需要补白：true为补白; false为不补白;
     */
    public final static BufferedImage scale2(BufferedImage bi, int width, int height, boolean bb) throws Exception {
        double ratio = 0.0; // 缩放比例
        Image itemp = bi.getScaledInstance(width, height, bi.SCALE_SMOOTH);
        // 计算比例
        if ((bi.getHeight() > height) || (bi.getWidth() > width)) {
     ...
     * @param height 目标切片高度
     */
    public final static BufferedImage cut(BufferedImage bi,int x, int y, int width, int height) throws Exception{
        int srcHeight = bi.getHeight(); // 源图宽度
        int srcWidth = bi.getWidth(); // 源图高度
        if (srcWidth > 0 && srcHeight > 0) {
            Image image = bi.getScaledInstance(srcWidth, srcHeight,
            ...
            //设置透明  end
            g2d.drawImage(img, 0, 0, width, height, null); // 绘制切割后的图
            g2d.dispose();
            return tag;
        }else{
            throw new ParamException("源图片错误");
        }
    }
